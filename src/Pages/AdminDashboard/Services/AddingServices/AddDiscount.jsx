...
export default function AddDiscount () {
  ...
  const handleSubmit = async e => {
    ...
    await fetch('/admin-page/create-discount', requestOptions)
      .then(result => {
  ...
  return (
    <div className='h-full mt-5 ml-2 mr-2 scroll-smooth scrollbar-hide overflow-y-scroll text-black'>
      <div className='mt-5 ml-5 text-white font-bold'>DODAJ NOVI POPUST</div>
      <div className='flex'>
        <form
        ...
        >
          ...
          <input
            ...
            onChange={e =>
              setData({
                ...data,
                aplTo: e.target.value.split(',').map(Number)
              })
            }
            placeholder='popust za usluge (1,2,3)'
            required
            className='mt-4 pl-2 rounded-3xl bg-green-800 bg-opacity-30 text-white focus:outline-none focus:bg-yellow-400 focus:bg-opacity-20'
          />
          <input
            ...
            onChange={e =>
              setData({
                ...data,
                aplToSub: e.target.value.split(',').map(Number)
              })
            }
            placeholder='popust za podusluge'
            required
            className='mt-4 pl-2 rounded-3xl bg-green-800 bg-opacity-30 text-white focus:outline-none focus:bg-yellow-400 focus:bg-opacity-20'
          />
          <div className='flex'>
