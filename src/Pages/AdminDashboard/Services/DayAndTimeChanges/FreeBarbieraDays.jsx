...
export default function FreeBarbieraDays () {
  ...
  const handleSubmit = async e => {
    ...
    await fetch('/admin-page/not-working', requestOptions)
      .then(result => {
  ...
  return (
    <div className='h-full w-full bg-black bg-opacity-40'>
      <h1 className='ml-5 pt-5'>DODAJ NERADNE DANE SALONA</h1>
      <form
      ...
      >
        <input
          ...
          placeholder='datum početka (YYYYY-DD-MM)'
          required
        ...
        <input
          ...
          placeholder='datum kraja (YYYYY-DD-MM)'
          required
          className='mt-4 pl-2 rounded-3xl bg-green-800 bg-opacity-30 text-white focus:outline-none focus:bg-yellow-400 focus:bg-opacity-20'
        />
        <button
